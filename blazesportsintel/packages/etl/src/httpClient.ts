...
async function readCache(filePath: string, ttlMs: number): Promise<string | null> {
  try {
...
async function writeCache(filePath: string, body: string): Promise<void> {
  try {
...
function isPathAllowed(robotsText: string, url: URL): boolean {
...
async function throttle(url: URL): Promise<void> {
  const host = url.host;
...
export async function fetchWithRespect(url: string, options: FetchWithRespectOptions = {}): Promise<string> {
